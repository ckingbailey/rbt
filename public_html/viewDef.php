...
$viewDefSql = file_get_contents("$baseDir/inc/sql/viewDef.sql");
...
if (!empty($_GET['defID'])) {
    ...
    try {
        ...
        while ($stmt->fetch()) {
            ...
            $twig->display('head.html.twig', [ 'title' => PROJECT_NAME . ' - Deficiency No. ' . $defID ]);
            echo "<body>";
    ...
    if ($result = $link->query('SELECT bdPermit from users where userID='.$_SESSION['userID'])) {
    ...
    if ($bdPermit) {
        ...
        if ($stmt = $link->prepare($sql)) {
            ...
            // query for comments associated with this Def
            $sql = "SELECT firstname, lastname, dateCreated, bdCommText
            ...
            if (count($comments)) {
                ...
                foreach ($comments as $comment) {
                    $timestamp = strtotime($comment['dateCreated']) - (60 * 60 * 7);
