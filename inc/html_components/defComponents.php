...
function returnCommentsHTML(array $comments) {
    ...
    foreach ($comments as $comment) {
        ...
        $content .= sprintf($commentFormat, $userFullName, $comment['dateCreated'], $text);
    }
...
$requiredElements = [
    ...
    'status' => [
        ...
        "type" => null,
        "query" => "SELECT statusID, statusName FROM status WHERE (statusName = 'open' OR statusName = 'closed') ORDER BY statusID",
        'value' => ''
    ],
    'severity' => [
    ...
    'milestone' => [
        "label" => returnLabel('milestone', 'Required for', 1),
        "tagName" => "select",
        "element" => "<select name='milestone' id='milestone' class='form-control' required>%s</select>",
        "type" => null,
        "query" => "SELECT milestoneID, milestoneName FROM milestone ORDER BY milestoneName",
        'value' => ''
    ],
    'contract' => [
        'label' => returnLabel('contract', 'Contract', 1),
        'tagName' => 'select',
        'element' => "<select name='contract' id='contract' class='form-control' required>%s</select>",
        'type' => null,
...
$closureElements = [
    ...
    'documentRepo' => [
        'label' => returnLabel('documentRepo', 'Evidence Repository'),
        'tagName' => 'select',
        'element' => "<select name='documentRepo' id='documentRepo' class='form-control'>%s</select>",
        'type' => '',
        'query' => "SELECT docRepoID, docRepoName FROM documentRepo ORDER BY docRepoName",
        'value' => ''
    ],
    'evidenceLink' => [
